...
public class ChipInterfaceEditor : InteractionHandler {
	...
	void UpdateSizeAndPos () {
		if (chipContainer) {
			transform.position = new Vector3 (transform.position.x, chipContainer.position.y, transform.position.z);
			transform.localScale = new Vector3 (transform.localScale.x, chipContainer.transform.localScale.y, 1);
		}
	}
