...
public class SavedChipEditor : MonoBehaviour {
	...
	public void Load (string chipName, GameObject chipHolder, Player player) {
		...
		var wiringSaveData = ChipLoader.LoadWiringFile (FindObjectOfType<SaveSystem> ().GetPathToWireSaveFile (chipName));
		int wireIndex = 0;
		foreach (var savedWire in wiringSaveData.wires) {
